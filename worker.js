  ...
  */
  accounting: {
    ...
    async _tenant(env, h) {
      ...
      const want = (env.ACCOUNTING_TENANT || '').trim();
      let org = null;
      if (want) org = list.find((c) => (c.tenantName || '').toLowerCase().includes(want.toLowerCase()) || (c.tenantId || '') === want);
      if (!org) org = list.find((c) => c.tenantType === 'ORGANISATION') || list[0];
